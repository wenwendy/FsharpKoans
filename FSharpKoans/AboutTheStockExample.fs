...
module ``about the stock example`` =
    ...
    [<Koan>]
    let SplitCommasTest() =
        let result = List.map splitCommas stockData
        let headArray = result.Head
        AssertEquality "Date" headArray.[0]
        AssertEquality "Open" headArray.[1]
        AssertEquality "Close" headArray.[4]

    let dateOpenClose (x:string[]) =
        [| x.[0]; x.[1]; x.[4]|]
        

    [<Koan>]
    let YouGotDateOpenCloseRight() =
        let result = List.map splitCommas stockData
          |> List.map dateOpenClose
        let resultHeadArray = result.Head
        AssertEquality "Date" resultHeadArray.[0]
        AssertEquality "Open" resultHeadArray.[1]
        AssertEquality "Close" resultHeadArray.[2]

    let dateVariance x = __


    [<Koan>]
    let YouGotDateVarianceRight x = __
        //let result = List.map dateVariance x

        //AssertEquality "2012-03-30,-0.36" result.Head
        //AssertEquality "2012-03-30,-0.36" List.nth result 2

    [<Koan>]
    let YouGotTheAnswerCorrect() =

        let isMaxVariance values = true
            //let mutable max = 0
        ...
        let maxVariance =
            ...
            |> List.map dateOpenClose
            |> List.map dateVariance
            //|> List.maxBy

        let maxVarianceArray = splitCommas maxVariance.Head
        let result = maxVarianceArray.[0]
